...
const DOMAINS = {
  ...
  dataGovernance: { title: 'Data governance', reads: ['data'], controls: ['APP-FIT-DATA-GOVERNANCE', 'APP-FIT-DATA-EXCHANGE-PURPOSE'], evaluate: (s) => ({ pass: !!(s.data && s.data.tracedRatio === 1), detail: s.data ? `${Math.round((s.data.tracedRatio ?? 0) * 100)}% of records fully traced` : 'no data-governance evidence' }) },
