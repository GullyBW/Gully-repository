...
async function route(app, req, url, body) {
  ...
  // --- Phase 12: executive quality, builder trust and deployability, AI fairness, session consistency ---
  if (method === 'GET' && p === '/api/data/quality/executive') { requireRole('oversight-board'); return json(200, app.fabric.dataGovernance.executiveQualityDashboard()); }
  if (method === 'GET' && p === '/api/supply-chain/builders') { requireRole('admin'); return json(200, { builders: app.supplyChain.attestation.builders().map((b) => app.supplyChain.attestation.verifyBuilder(b.id)), vulnerabilityTrend: app.supplyChain.attestation.vulnerabilityTrend() }); }
  if (method === 'POST' && p === '/api/supply-chain/deployability') { requireRole('admin'); return json(200, app.supplyChain.attestation.deployabilityReport(body || {})); }
  if (method === 'GET' && p === '/api/ai/fairness') { requireRole('oversight-board'); const l = app.ai.lifecycle; return json(200, { criteria: l.fairnessCriteria(), models: l.catalogue('model').map((m) => l.fairnessReport(m.id)) }); }
  ...
  if (method === 'GET' && p === '/api/governance/analytics') { requireRole('admin'); const f = [...runTwin(), ...runApp(), ...runInfra()].map((r) => ({ id: r.id, pass: r.pass })); const o = require('./governance/ownership'); return json(200, require('./architecture/drift-prevention').governanceAnalytics({ assumptions: app.assumptions, activity: o.activity, training: o.training, rehearsals: app.rehearsals, controls: f, now: Date.now() })); }
  ...
  if (method === 'POST' && (m = p.match(/^\/api\/architecture\/assumptions\/([^/]+)\/verify$/))) { const u = requireRole('admin'); return json(201, app.assumptions.recordVerification(dec(m[1]), { holds: body && body.holds, by: (body && body.by) || u.principal, detail: body && body.detail })); }
  if (method === 'GET' && p === '/api/twin/operations/confidence') { requireRole('admin'); const f = [...runTwin(), ...runApp(), ...runInfra()].map((r) => ({ id: r.id, pass: r.pass })); return json(200, app.operationsTwin().confidenceReport({ controls: f })); }
  ...
  // --- Phase 12: automated compliance intelligence, enterprise knowledge graph ---
  if (method === 'GET' && p === '/api/compliance/intelligence') { requireRole('admin'); const f = [...runTwin(), ...runApp(), ...runInfra()].map((r) => ({ id: r.id, pass: r.pass })); return json(200, app.legislation.intelligence.report({ controls: f, datasets: app.fabric.dataGovernance.datasets() })); }
  if (method === 'POST' && p === '/api/compliance/changes') { const u = requireRole('admin'); return json(201, app.legislation.intelligence.observe({ ...(body || {}), observedBy: (body && body.observedBy) || u.principal })); }
  if (method === 'POST' && (m = p.match(/^\/api\/compliance\/changes\/([^/]+)\/assess$/))) { const u = requireRole('oversight-board'); return json(200, app.legislation.intelligence.assess(dec(m[1]), { by: (body && body.by) || u.principal, conclusion: body && body.conclusion })); }
  // --- Phase 13: institutional chain, compliance lifecycle, temporal graph ---
  if (method === 'GET' && p === '/api/observability/mission-graph') { requireRole('oversight-board'); return json(200, require('./observability/business').missionDependencyGraph()); }
  if (method === 'GET' && p === '/api/compliance/lifecycle') { requireRole('admin'); const f = [...runTwin(), ...runApp(), ...runInfra()].map((r) => ({ id: r.id, pass: r.pass })); return json(200, { states: app.legislation.intelligence.complianceStates(), evolution: app.legislation.intelligence.evolution({ controls: f }) }); }
  if (method === 'POST' && (m = p.match(/^\/api\/compliance\/obligations\/([^/]+)\/state$/))) { const u = requireRole('oversight-board'); return json(200, app.legislation.intelligence.transition(dec(m[1]), { to: body && body.to, by: (body && body.by) || u.principal, rationale: body && body.rationale, independent: !!(body && body.independent) })); }
  if (method === 'GET' && (m = p.match(/^\/api\/compliance\/obligations\/([^/]+)\/timeline$/))) { requireRole('admin'); return json(200, app.legislation.intelligence.timeline(dec(m[1]))); }
  if (method === 'GET' && (m = p.match(/^\/api\/graph\/enterprise\/as-of\/(\d+)$/))) { requireRole('admin'); return json(200, app.assurance.enterpriseGraph().asOf(Number(m[1]), { node: url.searchParams.get('node') })); }
  if (method === 'GET' && p === '/api/graph/enterprise') { requireRole('admin'); return json(200, app.assurance.enterpriseGraph().report()); }
  if (method === 'GET' && (m = p.match(/^\/api\/graph\/enterprise\/impact\/([^/]+)$/))) { requireRole('admin'); return json(200, app.assurance.enterpriseGraph().impactOf(dec(m[1]))); }
  ...
  // --- Phase 12: context-aware authorization, quantitative risk, proof catalogue ---
  if (method === 'POST' && p === '/api/security/zero-trust/context') { requireRole('admin'); return json(200, app.iam.zeroTrust.pdp.evaluateContext(body.request || {})); }
  if (method === 'GET' && p === '/api/security/risk/quantitative') { requireRole('admin'); const f = [...runTwin(), ...runApp(), ...runInfra()].map((r) => ({ id: r.id, pass: r.pass })); const rr = app.threat.riskRegister; return json(200, { scales: { likelihood: require('./security/threat-model').LIKELIHOOD_SCALE, impact: require('./security/threat-model').IMPACT_SCALE }, quantitative: require('./security/threat-model').ids().map((id) => rr.quantitative(id, { fitnessResults: f })), treatments: rr.treatments(), forecast: rr.forecast() }); }
  if (method === 'GET' && p === '/api/security/formal-policy/proof-report') { requireRole('admin'); return json(200, app.iam.formalPolicy.proofReport({})); }
